...
`pixi run rebuild` — 全量重建数据库：

1. 将 `data/archive/` 中的账单文件移回 `data/input/`
2. 删除旧数据库 `data/api_ledger.db`
3. 重新运行主程序（导入 → 生成报告）

适用于预设规则变更（列映射 / 单价解析等）后，需要用新规则重算全部历史数据的情况。原 `rebuild_db.bat` 已由该任务替代。
